(function setup(window) {
    ...
    // Select canvas by ID and store UI elements
    window.can = document.querySelector("#game-canvas");
    window.ctx = window.can.getContext("2d");
    window.startButton = document.querySelector("#start-button");
    window.startScreen = document.querySelector("#start-screen");
    window.scoreDisplay = document.querySelector("#score-display");
    window.killCountElement = document.querySelector("#kill-count");
...
// Flag to control the game loop state
...
function startGame() {
    ...
    gameRunning = true;
    play(false); // Start the main game function
}

// Attach the event listener to the start button
window.startButton.on("click", startGame);
...
rand(-s * rand(2, 10), can.height + s * rand(2, 10));
    ...
    var frames = 0;
    (function update() {
      // Only run the game loop if the game is running
      if (!gameRunning) {
      ...
      for (let i in player.bullets) {
        ...
        for (let x in zombies) {
          ...
          if (p.x > z.x - z.s &&
            ...
            killCountElement.textContent = player.zombiesKilled.toLocaleString();
            // ---------------------------------
...
+ x * p.a * Math.cos(a), p.y + p.s * Math.sin(a + x * 30 * Math.PI / 180) 
...
Math.sin(a + x * 35 * Math.PI / 180) * p.s / 2, p.s / 4, 0, 2 * Math.PI);
      ...
      if (gameover) {
        gameRunning = false; // Stop game running flag
        ...
        ctx.beginPath();
        ctx.fillStyle = "red"; // Changed color for impact
...
2, 0.75 * can.height);
        ...
        // This is the WEAPON/AMMO UI, not the main score display
        ctx.beginPath();
        ctx.fillStyle = "black";
        ctx.textAlign = "left";
...
player.spray + "/" + player.magSize /
 player.spray, 0, 40);
    ...
    // Ensure event listeners only affect the game if it is running or needs restart
    can.on("mousedown", function() {
      if (!gameover && gameRunning) switch(player.weapon) {
        case "Machine Gun":
        case "Minigun":
        case "Laser Gun":
        player.fire = true;   
      ...
      if (gameover)
        switch(e.which || e.keyCode) {
          ...
          case 13:
            ...
            killCountElement.textContent = 0; // Reset score display
            gameRunning = true;
            play(false);
            // Re-attach the keydown listener to the window since play() will have removed it locally
            window.on("keydown", move);
            break;
        }
      else if (gameRunning) switch(e.which || e.keyCode) { // Check if game is running before processing movement
          case 37:
          case 65:
            player.mx = -1;
            break;
    ...
    // The initial call to play(false) when the script loads will be prevented by the gameRunning flag.
    // The event listeners for keydown/keyup are placed outside the play() function's closure so they persist
    // or are re-attached correctly on restart.
    window.on("resize", function() {
      ...
      .on("keyup", function(e) {
      if (!gameover && gameRunning) switch(e.which || e.keyCode) { // Check if game is running
        case 37:
        case 65:
        case 39:
        case 68:
